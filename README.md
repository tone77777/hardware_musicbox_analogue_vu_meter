...
- **GPIO Pin**: Uses GPIO 19 (BCM numbering) for PWM output - make sure nothing else is using this pin
...
- Analogue VU meter connected to GPIO 19 (PWM output)
- Power supply

### Hardware Connection
GPIO 19 (PWM) ----[ 10kΩ ]-----+----> VU Meter (+)
|
[10µF capacitor]
|
GND ------> VU Meter (−)

**Note:** GPIO 19 is used for hardware PWM output. The PWM signal is filtered through a 10µF capacitor and 10kΩ resistor to create a smooth DC voltage for the VU meter.
...
The VU meter reads raw PCM audio data from Squeezelite's shared memory file (`/dev/shm/squeezelite-<MAC-ADDRESS>`). It calculates the RMS (Root Mean Square) audio level and outputs a PWM signal on GPIO 19 proportional to the volume level.
...
- **GPIO output**: PWM signal on GPIO 19, range 0-500 (0-100% VU level)
...
Connect your analogue VU meter to GPIO 19 through a low-pass filter:

```
GPIO 19 (PWM) ----[ 10kΩ resistor ]-----+----> VU Meter (+)
                                         |
                                    [10µF capacitor]
                                         |
GND -------------------------------------+----> VU Meter (−)
...
4. **No PWM output**: Check GPIO 19 is not used by another process
